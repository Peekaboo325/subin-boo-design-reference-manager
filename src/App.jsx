...
const TYPE_OPTIONS = [
  "유형 선택",
  "온라인배너", "SNS피드", "SNS스토리", "유튜브썸네일",
  "랜딩페이지", "상세페이지", "팝업",
  "카드뉴스", "인포그래픽", "일러스트",
  "로고타이포", "그래픽에셋", "페인팅",
  "스튜디오컷", "PPT", "차트그래프", "UI포트폴리오",
  "기타"
];
...
- 아래 키워드 리스트에서 이미지와 정확히 맞는 키워드만 사용할 것
- 억지로 끼워맞추지 말 것 — 딱 맞는 키워드가 없으면 해당 카테고리를 비워두는 것이 나음
- 단, 리스트에 없지만 이미지를 명확하게 설명하는 키워드가 있다면 적극적으로 새로 만들어서 사용할 것
- 새로 만든 키워드는 반드시 키워드 뒤에 (NEW) 표시를 붙일 것
- (NEW) 표시는 파싱용으로만 사용되며 최종 출력에는 제거됨
...
async function saveToDrive(title, keywords, base64Full, mimeType, type, purpose) {
  const base64 = base64Full.split(",")[1];
  const response = await fetch("/api/save", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ title, keywords, imageBase64: base64, mimeType, type, purpose })
  });
  const data = await response.json();
...
export default function App() {
  ...
  const handleFiles = async (newFiles) => {
    ...
    const items = await Promise.all(imageFiles.map(async f => {
      ...
      return {
        ...
        file: f, preview, ratio: dims.w / dims.h,
        status: "pending", result: null, error: null,
        saveStatus: null, fileUrl: null,
        type: "", purpose: ""
      };
    }));
  ...
  const handleSave = async (item) => {
    ...
    try {
      const fileUrl = await saveToDrive(
        item.result.title,
  ...
  const updateMeta = (id, field, value) => setFiles(prev => prev.map(f => f.id === id ? { ...f, [field]: value } : f));
                  ...
                  {item.status === "done" && item.result && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                      {[
                        ...
                        <div key={key} style={{ background: "#111", borderRadius: "8px", overflow: "hidden" }}>
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", borderBottom: "1px solid #1e1e1e" }}>
                            ...
                            <button onClick={() => handleCopy(item.id, key, field === "keywords" ? item.result[field].replace(/\s*\(NEW\)/g, "") : item.result[field])}
                          ...
                          <div style={{ padding: "10px 12px" }}>
                            <EditableField value={field === "keywords" ? item.result[field].replace(/\s*\(NEW\)/g, "") : item.result[field]}
                              onChange={v => updateResult(item.id, field, v)}
                      ...
                      <div style={{ display: "flex", gap: "8px" }}>
                        <select
                          value={item.type}
                          onChange={e => updateMeta(item.id, "type", e.target.value)}
                          style={selectStyle}
                        >
                          {TYPE_OPTIONS.map(opt => (
                            <option key={opt} value={opt === "유형 선택" ? "" : opt}>{opt}</option>
                          ))}
                        </select>
                        <select
                          value={item.purpose}
                          onChange={e => updateMeta(item.id, "purpose", e.target.value)}
                          style={selectStyle}
                        >
                          {PURPOSE_OPTIONS.map(opt => (
                            <option key={opt} value={opt === "목적 선택" ? "" : opt}>{opt}</option>
                          ))}
                        </select>
                      </div>
